...
class TestUserEndpoint(unittest.TestCase):
    ...
    def test_01_create_user(self):
        ...
        response = requests.post(
            self.url + "users/", json=user_data, headers=self.headers
        )
        self.assertEqual(response.status_code, 201)
        response_data = response.json()
        user = User.query.filter_by(email="test@gmail.com").first()
        self.assertIsNotNone(
            user, "L'utilisateur devrait exister dans la base de données."
        )
        self.assertTrue(
            bcrypt.check_password_hash(user.password, user_data["password"]),
    ...
    def test_03_test_login(self):
        login_data = {"email": "test@gmail.com", "password": "test"}
        response = requests.post(
            self.url + "auth/login", json=login_data, headers=self.headers
        )
        ...
        self.assertIn(
            "access_token",
            response_data,
            "L'access token est manquant dans la réponse.",
        )
    ...
    def test_04_test_protected(self):
        ...
        self.assertEqual(
            response.status_code, 200, "Accès à l'endpoint protégé échoué."
        )
    ...
    def test_05_test_unauthorized(self):
        ...
        self.assertEqual(
            ...
            422,
            "Accès à l'endpoint protégé a réussi alors que cela ne devrait pas.",
        )
    ...
    def test_06_test_create_place_without_valid_token(self):
        ...
        TestUserEndpoint.unauthenticated_headers = {
            "Content-Type": "application/json",
            "Authorization": f"Bearer {TestUserEndpoint.token}",
        }

        response = requests.post(
            self.url + "places/",
            json=place_data,
            headers=TestUserEndpoint.unauthenticated_headers,
        )
        print(response.json())
